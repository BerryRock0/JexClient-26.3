...
public class SpeedMine extends Feature {
    ...
    private final EventListener<EventClickBlock> eventClickBlockEventListener = new EventListener<>(event -> {
        ...
        if (!WorldHelper.INSTANCE.isBreakable(WorldHelper.INSTANCE.getBlock(event.getBlockPos())))
            return;
        for (int i = 0; i < 10; i++) {
            NetworkHelper.INSTANCE.sendPacket(new PlayerActionC2SPacket(ServerboundPlayerActionPacket.Action.STOP_DESTROY_BLOCK, event.getBlockPos(), event.getFace()));
            NetworkHelper.INSTANCE.sendPacket(new PlayerActionC2SPacket(ServerboundPlayerActionPacket.Action.START_DESTROY_BLOCK, event.getBlockPos(), event.getFace()));
        }
        Wrapper.INSTANCE.getIClientPlayerInteractionManager().setBlockBreakProgress(1);

...
public class EnchantmentArgumentType implements ArgumentType<Enchantment>
{
   ...
   {
      ResourceLocation identifier = Identifier.fromCommandInput(stringReader);
      return Registry.ENCHANTMENT.getOrEmpty(identifier).orElseThrow(() -> {return UNKNOWN_ENCHANTMENT_EXCEPTION.create(identifier);});
   }

   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder)
   {
      Registry.ENCHANTMENT.getIds().forEach(identifier -> {builder.suggest(identifier.toString());});
